...
def __is_game_name_predicate(predicate_fn):
...
def __is_round_name_predicate(predicate_fn):
...
def get_choices(predicate_str, draft):
    ...
    try:
        ...
        if __is_game_name_predicate(pred):
            ret = __get_game_names()
        if __is_level_name_predicate(pred):
            ret = __get_level_names(draft=draft)
        if __is_resource_predicate(pred):
            ret = __get_resources()
        if __is_round_name_predicate(pred):
            ret = __get_round_names()
    except KeyError:
        pass
    return ret
