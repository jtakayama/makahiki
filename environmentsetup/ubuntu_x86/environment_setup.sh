...
# virtualenvwrapper.
# Target operating system: Ubuntu Linux (32-bit).
	...
	then
	    if [ -f /usr/lib/i386-linux-gnu/libjpeg.so ]
			then
				...
				echo "Creating symlink: /usr/lib/i386-linux-gnu/libjpeg.so --> /usr/lib/"
				echo "sudo ln -s /usr/lib/i386-linux-gnu/libjpeg.so /usr/lib/"
...
if [ ! -f /usr/lib/libz.so ]
	then
		if [ -f /usr/lib/i386-linux-gnu/libz.so ]
			then
				"Found: /usr/lib/i386-linux-gnu/libz.so"
				echo "Creating symlink: /usr/lib/i386-linux-gnu/libz.so --> /usr/lib/"
