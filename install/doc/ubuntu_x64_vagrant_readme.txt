...
- Operating System:
  ...
  - An x64 (64-bit) OS is required to run x64 virtual machines.
  - On Windows, you will need to enable hardware virtualization in the BIOS 
    in order to use 64-bit virtual machines. The process is different for 
    each PC manufacturer and CPU, and is not within the scope of this guide.
- Hardware:
...
uses the example directory "ubuntu_x64_makahiki." A Vagrant virtual machine 
...
> mkdir ubuntu_x64_makahiki
> cd ubuntu_x64_makahiki
===============================================================================
...
B. If you have Git for Windows, you can clone the repository:
   B1. If you are not in it, change to the "ubuntu_x64_makahiki" directory.
...
ubuntu_x64_makahiki directory from the makahiki directory:

> cd makahiki\install
> copy_ubuntu_x64_scripts.bat
> cd ../../

On the Vagrant virtual machine, the ubuntu_x64_makahiki directory will be 
...
"precise64," for Vagrant to use. (Replace "precise64" with whatever 
...
virtual machines with the same base box ("precise64"), you can skip this step.
===============================================================================
...
vagrant@precise64:~$ cd makahiki

Then, create the makahiki virtual environment: 

vagrant@precise64:~/makahiki$ mkvirtualenv makahiki
...
(makahiki)vagrant@precise64:~/makahiki sudo sh /vagrant/md5test_ubuntu_pg_hba_conf.sh
...
(makahiki)vagrant@precise64:~/makahiki$ sudo /etc/init.d/postgresql restart
...
(makahiki)vagrant@precise64:~/makahiki$ sudo nano /etc/postgresql/9.1/main/pg_hba.conf
...
with Makahiki settings:
(makahiki)vagrant@precise64:~/makahiki sh /vagrant/md5test_ubuntu_postactivate.sh
...
(makahiki)vagrant@precise64:~/makahiki$ workon makahiki
...
(makahiki)vagrant@precise64:~/makahiki$ nano $WORKON_HOME/makahiki/bin/postactivate
...
(makahiki)vagrant@precise64:~/makahiki$ cd makahiki
...
(makahiki)vagrant@precise64:~/makahiki/makahiki$ ./manage.py runserver 0.0.0.0:8000

To start the server with gunicorn:
(makahiki)vagrant@precise64:~/makahiki/makahiki$ ./manage.py run_gunicorn -b 0.0.0.0:8000
...
In the ubuntu_x64_makahiki folder, start the virtual machine with "vagrant up."
...
(makahiki)vagrant@precise64:~/makahiki/makahiki$ sudo shutdown -h now
...
vagrant@precise64:~$ workon makahiki
(makahiki)vagrant@precise64:~$ cd makahiki/makahiki
(makahiki)vagrant@precise64:~/makahiki/makahiki$ ./manage.py runserver
===============================================================================